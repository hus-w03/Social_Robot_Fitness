...
async def write_loop():
    while True:
        await asyncio.sleep(1)  # updates the file every 5 seconds
        write_to_file()
...
def sensor_callback(msg):
    global ros_client, sensor_connected
    resp_topic = roslibpy.Topic(ros_client, "/ros_resp", 'std_msgs/String')
    while not sensor_connected:
        time.sleep(0.5)
    resp_topic.publish(roslibpy.Message({'data': "sensor_connected"}))


async def main():
    global hr_data_q, data_out, cur_hr_data, ros_client
    cur_hr_data = []
    hr_data_q = Queue(maxsize=120)

    ros_client = roslibpy.Ros("localhost", 9091)
    ros_client.run()
    while not ros_client.is_connected:
        time.sleep(0.5)
    sensor_topic = roslibpy.Topic(ros_client, "/sensor_connected", 'std_msgs/String')
    sensor_topic.subscribe(sensor_callback)
