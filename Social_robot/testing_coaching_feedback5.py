...
HRV_CONFIG = {
    ...
    "window_size": 10,         # Rolling window for RMSSD analysis
}
...
class EnhancedExerciseState:
    def __init__(self):
        ...
        self._rmssd_history = deque(maxlen=HRV_CONFIG["window_size"])
...
def print_hrv_status(rmssd, recovery_percent=None):
    """Print HRV status with recovery information"""
    timestamp = datetime.now().strftime("%H:%M:%S")
    if recovery_percent:
...
def assess_hrv_recovery():
    """Assess current HRV recovery status compared to baseline"""
    if state.baseline_rmssd is None or state.current_rmssd <= 0:
        return None
    
    recovery_ratio = state.current_rmssd / state.baseline_rmssd
...
def get_adaptive_rest_feedback():
    """Provide feedback during adaptive rest period"""
    ...
    if recovery["recovered"]:
        ...
        return f"🟡 Good recovery progress ({percent:.1f}%). Almost ready..."
    elif status == "moderate":
        return f"🟠 Moderate recovery ({percent:.1f}%). Keep resting..."
    else:
        if rest_duration > 90:  # If resting for a while with poor recovery
            return f"🔴 Recovery slower than expected ({percent:.1f}%). This is normal - your body is working hard!"
        else:
            return f"🔴 Early recovery phase ({percent:.1f}%). Let your nervous system reset..."
